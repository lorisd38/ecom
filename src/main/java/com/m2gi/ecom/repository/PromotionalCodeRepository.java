...
public interface PromotionalCodeRepository extends JpaRepository<PromotionalCode, Long> {
    ...
    @Query(
        "select distinct promotionalCode from PromotionalCode promotionalCode left join fetch promotionalCode.products " +
        " where promotionalCode.code = :code and :date between promotionalCode.startDate and promotionalCode.endDate"
    )
    Optional<PromotionalCode> findActiveWithEagerRelationshipsByCode(@Param("code") String code, @Param("date") Instant date);
}
